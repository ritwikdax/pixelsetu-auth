...
export const checkNamespaceAvailabilitySchema = z.object({
  namespace: z.string().min(3, { message: "Namespace must be at least 3 characters long" }).max(30, { message: "Namespace must be at most 30 characters long" }).regex(/^[a-zA-Z0-9_-]+$/, { message: "Namespace can only contain letters, numbers, underscores, and hyphens" })
}).strict();

export const businessFormDataSchema = z.object({
  displayName: z.string().min(1, { message: "Display name is required" }),
  namespace: z.string().min(3, { message: "Namespace must be at least 3 characters long" }).max(30, { message: "Namespace must be at most 30 characters long" }).regex(/^[a-zA-Z0-9_-]+$/, { message: "Namespace can only contain letters, numbers, underscores, and hyphens" }),
  lightLogoUrl: z.url({ message: "Invalid URL" }).or(z.literal("")).optional(),
